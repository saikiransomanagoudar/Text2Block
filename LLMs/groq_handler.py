...
class GrokHandler:
    ...
    def generate_dot_code(self, user_prompt):
        ...
        prompt = (
            ...
            f"query = {user_prompt} + in detail \n\n"
            f"Ensure the following:\n"
            f"1. The node names should not contain spaces. If the node name consists of multiple words, combine them into a single word without spaces (e.g., 'RandomVariablesAndPDF').\n"
    ...
    def generate_text_response(self, dot_code):
        ...
        """
        prompt = (
            f"Based on the following DOT code for a flowchart, generate a brief textual explanation (under 300 words):\n\n{dot_code}\n\n"
